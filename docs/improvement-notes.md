...
백엔드 변경 없이 `DataTable` 컴포넌트에 TanStack Table의 `getPaginationRowModel()`을 적용하여 클라이언트 사이드 페이지네이션을 구현했다.

- `pageSize?: number` prop 추가 (기본값 20, 뷰별 오버라이드 가능)
- 페이지당 행 수 선택 (10 / 20 / 50 / 100)
- 이전/다음 페이지 이동 버튼 + 현재 페이지 표시
- 검색 결과 변경 시 자동으로 첫 페이지 리셋
- 순번(#)이 전체 기준 연속 번호로 표시 (예: 2페이지 1번 행 → 21)
- 모든 화면에 자동 적용 (뷰 파일 수정 불필요)

서버 사이드 페이지네이션은 데이터 규모가 실제 문제가 되는 시점에 백엔드와 함께 도입한다.
