          ...
          <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-5">
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <label class="block text-sm font-medium text-gray-700">입고 상세</label>
                <button
                  v-if="!isLocked"
                  type="button"
                  @click="addLine"
              ...
              <p v-if="lines.length === 0" class="text-sm text-gray-500 py-3 text-center">입고 상세를 추가하세요.</p>
            </div>
    ...
    if (open) {
      ...
      if (props.initialData) {
        header.value = {
          ...
          receiptDate: props.initialData.receiptDate,
          poId: props.initialData.poId,
        ...
        lines.value = [
          { itemId: 0, receiptTypeCode: 'GR_LINE_TYPE_01', receivedQuantity: 0, unitPrice: null, remarks: '', poLineId: null },
        ]
      }
    }
  },
...
function addLine() {
  lines.value.push({
    itemId: 0,
    receiptTypeCode: 'GR_LINE_TYPE_01',
    receivedQuantity: 0,
    unitPrice: null,
    remarks: '',
    poLineId: null,
...
function handleSubmit() {
  ...
  const validLines = lines.value.filter((l) => l.itemId > 0 && (l.receivedQuantity || 0) > 0)
  if (validLines.length === 0) {
    internalError.value = '품목이 선택된 입고 상세를 1개 이상 추가해야 합니다.'
    return
  }
  internalError.value = ''

  const lineRequests: GoodsReceiptLineRequest[] = validLines.map((l, i) => ({
    itemId: l.itemId,
    poLineId: l.poLineId,
    receiptTypeCode: l.receiptTypeCode,
    receivedQuantity: l.receivedQuantity,
  ...
  const payload: GoodsReceiptRequest = {
