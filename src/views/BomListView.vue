<template>
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-bold text-gray-900">BOM 관리</h2>
        <p class="text-sm text-gray-500 mt-0.5">자재명세서(BOM)를 조회하고 등록합니다.</p>
      </div>
      <button
    ...
    <!-- 그리드 -->
    <DataTable :data="rows" :columns="columns" :loading="loading" table-id="bom-list">
      <template #actions="{ row }">
        <button
          @click="router.push(`/bom/${row.original.id}`)"
...
/**
 * BOM 목록 화면 (화면 1).
 * BOM 검색/목록 표시 및 신규 등록 진입점.
 * 행 클릭 시 BOM 상세 화면(화면 2)으로 이동한다.
 */
...
const rows = ref<BomDto[]>([])
const loading = ref(false)
const modalOpen = ref(false)
const editTarget = ref<BomDto | null>(null)
...
const search = reactive({ itemCode: '', itemName: '', version: '', activeYn: '' })
...
const searchFields: SearchFieldDef[] = [
  ...
  { key: 'version', label: '버전', placeholder: '버전 검색' },
...
async function loadData() {
  ...
  try {
    ...
    showError('BOM 목록을 불러오는 중 오류가 발생했습니다.')
  } finally {
    loading.value = false
  }
}
...
// 컬럼 정의
const columnHelper = createColumnHelper<BomDto>()
const columns = [
  ...
  columnHelper.accessor('version', { header: '버전', enableSorting: true }),
  columnHelper.accessor('lineCount', { header: '자재 수', enableSorting: false }),
  columnHelper.accessor('validFrom', {
    header: '유효기간',
    enableSorting: false,
    cell: (info) => {
      const from = info.row.original.validFrom
      const to = info.row.original.validTo
      if (!from && !to) return '-'
      return `${from ?? '?'} ~ ${to ?? '?'}`
    },
  }),
